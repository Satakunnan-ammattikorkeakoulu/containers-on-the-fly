...
Create the ServerLogs and ServerStatus tables for real-time container server
monitoring. ServerLogs stores per-server log entries keyed by (computerId,
logType) with a unique constraint. ServerStatus tracks live health metrics
for each container server including CPU, memory, disk usage, Docker container
counts, system load averages, and uptime.
...
def upgrade() -> None:
    """Create ServerLogs and ServerStatus tables with foreign keys to Computer."""
...
def downgrade() -> None:
    """Drop the ServerStatus and ServerLogs tables."""
    # ### commands auto generated by Alembic - please adjust! ###
