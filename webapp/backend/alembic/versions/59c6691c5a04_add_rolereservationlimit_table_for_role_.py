...
Create the RoleReservationLimit table to enforce per-role constraints on
reservations. Each row ties a Role to optional minimum/maximum duration
(in minutes) and a maximum number of concurrent active reservations. A
unique constraint on roleId ensures one limit record per role.

Columns: roleReservationLimitId (PK), roleId (FK -> Role, unique),
minDuration, maxDuration, maxActiveReservations, createdAt, updatedAt.
...
def upgrade() -> None:
    """Create the RoleReservationLimit table with a foreign key to Role."""
...
def downgrade() -> None:
    """Drop the RoleReservationLimit table."""
    # ### commands auto generated by Alembic - please adjust! ###
