...
def send_email(to, mail_subject, mail_body):
    """Send a plain-text email asynchronously in a background thread.

    The SMTP operation runs in a daemon thread so it does not block the
    caller. Does nothing if email sending is disabled in settings or if
    required SMTP settings are incomplete. Uses SSL/TLS for port 465 and
    STARTTLS for other ports (typically 587). Errors are logged rather
    than raised.
    ...
    Returns:
        The background Thread object (can be joined in tests for
        deterministic assertions). Production callers can ignore it.
    """
