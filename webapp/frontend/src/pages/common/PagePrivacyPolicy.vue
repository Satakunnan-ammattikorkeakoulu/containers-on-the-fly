...
<script>
  /**
   * Privacy policy page. Fetches Markdown content from the legal API
  ...
  export default {
    ...
    methods: {
      ...
      fetchContent() {
        ...
        axios.get(this.$appSettings.APIServer.legal.get_privacy_policy)
          .then((response) => {
            if (response.data && response.data.status) {
              ...
              this.error = true
              this.errorMessage = response.data?.message || 'Privacy policy is not available.'
            }
          })
          .catch(() => {
            this.error = true
            this.errorMessage = 'Could not load the privacy policy.'
          })
          .finally(() => {
            this.loading = false
          })
